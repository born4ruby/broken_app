class DefaultMailer < ActionMailer::Base
  default from: "postmaster@transactionmanagement.com"
  ...
  def send_pass(user, pass)
    @user = user
    @pass = pass
    mail(:to => @user.email, :subject => "Welcome to app.transactionmanagement.com")
  end
  ...
  def invitation_mail(user, invite, email=nil)
    ...
    @invite = invite
    @user = invite.user
    mail(:to => email.nil? ? @user.email : email, :subject => "#{@from_user.fullname} of #{@from_user.company.name} has invited you to be added to his account at TransactionManagement.com")
  end
